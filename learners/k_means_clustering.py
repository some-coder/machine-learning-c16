...
class KMeansClustering(Learner):
    ...
    def __init__(self, k: int, **params: any):
        """
        Initialises K-means clustering learning algorithm.
    ...
    def fit(self, rs: RecordSet) -> None:
        ...
        the datapoints no longer change clusters
        ...
        list_of_datapoints = np.array_split(self.data.entries, self.k)# the data set is now split into K clusters
        codebooks = []
        for cluster in list_of_datapoints:
        ...
        while i < 100:  # max 100 iterations before stopping
            ...
            for cluster in new_list:
                try:
                    ...
                    pass
                    #print("this cluster is empty, it is not appended to the codebook anymore")
                else:
                    codebooks.append(np.mean(cluster[:, :], axis=0))  # mean per column (including outcome)
            codebook_array = np.array(codebooks)
            ...
            if np.array_equal(old[old[:,1].argsort()], new[new[:,1].argsort()]):    # if the codebook vectors don't change anymore, we're done
                break
            old_codebook_array = codebook_array
            i += 1
